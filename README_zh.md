...
### 1. 为什么每次更新版本后，划词功能失效或提示需要辅助功能授权？

由于 macOS 系统的严格安全机制，辅助功能（Accessibility）权限与应用程序二进制文件的数字签名/哈希值强绑定。由于开源发布包未购买昂贵的 Apple 官方付费开发者证书公证，在每次升级更新或覆盖安装后，macOS 会将新文件视作全新程序，导致系统层面的辅助功能授权失效。

**解决方法**：

1. 打开 macOS **「系统设置」** ➔ **「隐私与安全性」** ➔ **「辅助功能」**。
2. 在列表中找到 **ParrotLingo**，将其开关**关闭后再重新打开**（或点击下方的 `-` 移除后重新点击 `+` 添加 `/Applications/ParrotLingo.app`）。
3. 重新启动 ParrotLingo，即可恢复全局极速划词与悬浮胶囊工具条功能。

### 2. macOS 提示「应用已损坏，应将它移到废纸篓」怎么办？
